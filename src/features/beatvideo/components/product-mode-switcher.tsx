...
} from '@/features/beatvideo/product-mode'
...
export function BeatvideoModeSwitcher(props: {
  ...
  const changeMode = async (nextMode: BeatvideoProjectMode) => {
    ...
    try {
      ...
      // Product mode changes the visible workflow, not the underlying edit model.
      // Return to the normal edit surface and let mode-aware panels decide what
      // should be visible.
      setWorkspace('edit')
      setActiveTab('media')
    } catch (error) {
